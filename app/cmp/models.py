...
class ComprasEnc(ClaseModelo):
    ...
    class Meta:
        verbose_name_plural="Encabezadp Compras"
        verbose_name="Encabezadp Compra"
...
def detalle_compra_borrar(sender, instance, **kwargs):
    ...
    if enc:
        sub_total=ComprasDet.objects.filter(compra=id_compra).aaggregate(Sum('sub_total'))
        descuento= ComprasDet.objects.filter(compra=id_compra).aaggregate(Sum('descuento'))
